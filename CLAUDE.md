...
data/nfip.json              # committed (~340KB): R5 county×year NFIP claims rollup (counts/paid/byYear/SFHA-in-out per county+state). NOT yet surfaced in the UI (data + monthly refresh first; Geography "Flood insurance" lens is the next step). Refreshed MONTHLY by .github/workflows/refresh-monthly.yml.
