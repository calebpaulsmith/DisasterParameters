...
data/disasters.json         # 83 Region 5 disasters (FY2007–2026) — the source of truth (committed)
...
scripts/build_program_years.py # OFFLINE: builds data/program_years.json — the "Programs by Year" tab's dataset: ONE year x state x PROGRAM cube for Region 5 across all ten programs (pa, ihp, hmgp, mit, empg, prepFire/prepHomeland/prepTransit/prepNonprofit/prepOther — the SAME taxonomy the Geography lenses use). Each cell carries $ + a within-year drill-down: sub-program split (PA damage category A-G/Z · HMGP/mit project type or program · IHP HA vs ONA · grant sub-program), top recipients, and the disasters behind it. YEAR BASIS differs per program and is labeled in the UI: PA (Details v2 lastObligationDate), HMGP + non-disaster HMA (v4 initialObligationDate) = TRUE obligation year; EMPG/preparedness = grant fiscal year; IHP = PROXY incident year (no obligation date exists). PA applicant NAMES come from the PublicAssistanceApplicants v1 (dn,applicantId) bridge (Details carries no name); recipient-name variants are grouped with dedup_applicants.normalize() (imported — one normalizer for the repo). ONLY federalShareObligated counts: HMA rows with no obligated federal share are project REQUESTS (denied/pending/not selected) and are excluded, their requested $ reported in audit.applicationsNotObligated — do NOT reintroduce the projectAmount fallback that county_declarations' mit series uses. Every pull is GATED on the server's own $inlinecount ($top/$skip pagination silently truncates on a dropped page — that is how the committed county_declarations IL preparedness series ended up ~$213M short); a short pull ABORTS without writing. A scope-exact "R5" pseudo-state cell is written alongside the six states so region-wide top-recipient lists are exact, not a merge of truncated per-state lists. Audit reconciles every program against county_declarations' *ByYear buckets and STATES THE CAUSE of each delta, plus a PA worksheets(Details)-vs-ledger(FemaWebDisasterSummaries) cross-dataset check (they differ most on recent disasters — e.g. DR-4880 MI). Needs network; resumable cache (_progyears_cache.json).
data/program_years.json     # committed (~515KB), fetched LAZILY by the Programs by Year view only: {states, names (dictionary-encoded), years{YYYY:{ST|R5:{prog:{a,n,sub,top,nr,dis,nd,ns,m406}}}}, disasters, ledgerYear0, audit}. Powers the stacked obligations-by-year chart (state filter · program-group filter · legend toggles · click-a-year drill-down) and the per-program state-comparison chart (total or per capita). Pre-FY2007 years exist for the grant programs only (the disaster ledger starts at FY2007) and are hidden behind a "show the full record" toggle with an on-chart marker, so a year is never read as "no PA" when it is really "not covered".
...
scripts/add_history.py      # OFFLINE: pulls older R5 disasters (FEMA costs + hazards) and merges them in. ALSO the way to add NEWLY-DECLARED disasters (e.g. `add_history.py 2026 2026`): needs data/_disasters_raw.json regenerated first (gitignored — rebuild via its pull_declarations over FY2007–now) + the StormEvents CSV for the incident year in data/. Then run build_declared.py + add_new_disasters_geo.py.
scripts/add_new_disasters_geo.py # OFFLINE, ADDITIVE: after add_history.py adds new disasters to the ledger, this patches the GEOGRAPHY side for exactly those disasters WITHOUT the heavy Tier-1b county pipeline: county designations (from _disasters_raw) + per-county PA worksheets + per-county IHP registrant pull → county_declarations.json (fields, byYear buckets, ihpAudit conservation, state rollups) AND disaster_county_ihp.json (byDisaster + audit). Idempotent (skips disasters already present); asserts dollar conservation + date-descending disaster lists. Needs network.
scripts/refresh_ledger_costs.py # OFFLINE: re-pulls the AUTHORITATIVE costs (FemaWebDisasterSummaries + PA project $inlinecount) for EVERY ledger disaster — obligations reconcile for years, so recent rows go stale (DR-4880 MI PA sat at $12.4M while FEMA's figure had grown to $64.2M) — and propagates into county_declarations.json: DR-wide pa/ihp on each county's disasters[] entries, ledger-sourced state rollups, ihpAudit LEDGER side + flags, and the ihpByYear proxy buckets. Touches ONLY costs — hazards/gages/identity/registrant-side county $ are left alone (cross-dataset residuals stay audited). Needs network (~2 requests/disaster).
...
- **IA ≠ IHP.** **IA** (Individual Assistance) is the umbrella *authorization*; **IHP**
  ...
  **IHP** (the program that actually carries public $); the *registrations* count is labeled **IA**
  ...
  `iaDeclared = ihpDeclared = 37` and `(ihpTotal>0) = 36` (DR-4933 is IHP-declared with no
  approved $ recorded yet — recent disasters lag) and `iaProgramDeclared = 8` — but that
  reconciliation is an **observed property of current data, not a universal rule** (it breaks for
  legacy/pre-IHP records and for freshly-declared disasters).
...
- **`build_state_prep.py` silently truncates a state on a dropped page — FOUND, NOT FIXED.**
  Its `pages()` helper ends pagination when a fetch returns `None`, so one transient failure
  quietly shortens that state's series. The committed `county_declarations.json` currently
  carries the result: **Illinois preparedness is ~$213M short** (`afg`/`prepByFamily` and their
  `*ByYear` series). `scripts/build_program_years.py` pulls the same datasets with a
  `$inlinecount` gate and reports the gap in `data/program_years.json`'s
  `audit.reconcile.prepFire`, so the Programs-by-Year figures are complete while the Geography
  ones are not. Fix is small (gate `pages()` on the server's record count and raise instead of
  `break`), but it requires re-running the heavy county pipeline to correct the committed data —
  left for the Tier 1b rebuild below rather than done in passing.
